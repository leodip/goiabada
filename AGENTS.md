...
- `hashutil/` - SHA-256 over a string, and the check of one against its hash. It stayed here when password hashing went to `authserver/internal/passwordhash`, because both processes reach it independently: the auth server hashes authorization, verification and reset codes to locate rows, the admin console hashes the nonce it sends and re-checks it (#360)
