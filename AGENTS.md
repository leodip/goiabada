...
8. **Logging**: `core/logging` owns the one `slog` handler, installed by both servers at startup. Every message is a lowercase string literal with no `component:` prefix and no `failed to` or `error ` opener, the one verb being "unable to"; values ride as snake_case attributes, one fixed name per concept, `error` for an error, `client_identifier` for the wire string against `client_id` for the row id. `request_id` is the handler's, injected from the context, so a site with a `context.Context` or `*http.Request` in scope at any depth uses the `*Context` variant. Error means someone must act, Warn a refused or handled request, Info lifecycle and configuration, Debug per-request tracing. `TestSlogConvention` in `core/testutil/slog_lint.go`, called from all three module tiers, refuses every rule above decidable from the text, including keys a wrapper forwards on a caller's behalf, plus `slog.SetDefault`/`New`/`Default`/`With` outside the handler's own files; level is not decidable and is pinned per site through `testutil.CaptureSlog(t)`, the one way a test reads records (#320)
