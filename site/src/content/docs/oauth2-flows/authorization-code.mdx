...
| `nonce` | Recommended. Random string included in the ID token to prevent replay attacks |
| `prompt` | Controls authentication and consent behavior (`none`, `login`, `consent`). See [Prompt parameter](/concepts/prompt-parameter/) |
| `max_age` | Maximum acceptable time in seconds since the user last authenticated |
| `ui_locales` | Preferred languages for the login and consent screens (space-separated BCP 47 tags, e.g. `pt-BR en`). See [Localization](/concepts/localization/) |
| `acr_values` | Requested authentication level. See [ACR and AMR](/concepts/acr-amr/) |
...
5. Configure the **PKCE requirement** for a confidential client (public clients always require PKCE)

### PKCE configuration

For confidential clients, PKCE can be configured at two levels:
...
- `login_required` - User authentication is required (typically with `prompt=none`)
- `consent_required` - User consent is required (typically with `prompt=none`)
- `interaction_required` - User interaction is required, such as ACR step-up (typically with `prompt=none`)

These arrive once the user has signed in. Goiabada does not redirect an unauthenticated browser to a client's `redirect_uri`, so a request that fails validation before anyone has logged in sends the user to the login page first and delivers the error afterwards, unchanged. A browser that already holds a session, and any `prompt=none` request, is answered immediately. See [Deferred error redirects](/concepts/authorization-lifecycle/#deferred-error-redirects).

One failure never arrives at all, whoever is signed in: a `response_mode` Goiabada does not support. Understanding the requested mode is what tells the server how to encode a response, so a request asking for one it cannot speak is answered with `400 Bad Request` on a page, carrying no `error`, `error_description` or `state`. Supported values are `query`, `fragment` and `form_post`.

A client that [registered itself](/concepts/clients/#dynamic-client-registration-rfc-7591) receives none of these. Goiabada withholds its error redirects and shows the user a page instead, so an authorization that fails never returns to the client at all.

### Token errors

Errors during token exchange return a JSON response with cache prevention headers per [RFC 6749 Section 5.2](https://datatracker.ietf.org/doc/html/rfc6749#section-5.2):
