...
These checks run first. The client and `redirect_uri` checks are also *answered* first, on a page; the rest are answered by a redirect to the client, and a redirect is never sent to a browser that has not authenticated (see [Deferred error redirects](#deferred-error-redirects) below).
...
<Aside type="caution">
If the client or redirect URI is invalid, the error is shown on a page. Goiabada will **never** redirect to an unregistered URI. All other validation errors reach the client at its `redirect_uri`, but not necessarily straight away.
...
So a validation failure from the table above is answered one of two ways:
