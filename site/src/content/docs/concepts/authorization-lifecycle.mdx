...
1. If `acr_values` is in the authorization request, take the first value, then raise it to the client's default ACR level if it is below it
...
|-----------|----------------|
| `acr_values` provided in request | The higher of the first value from the `acr_values` parameter and the client's default ACR level |
