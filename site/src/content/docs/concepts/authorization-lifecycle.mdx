...
1. If `acr_values` names at least one recognised value, take the first value it recognises, then raise it to the client's default ACR level if it is below it
2. Otherwise, meaning the parameter is absent or names no value Goiabada recognises, use the client's default ACR level
...
|-----------|----------------|
| `acr_values` names at least one recognised value | The higher of the first recognised value from the `acr_values` parameter and the client's default ACR level |
| `acr_values` not provided, or none of its values recognised | Client's default ACR level |

The client's default ACR level is a floor. The `acr_values` parameter lets an application request a stronger authentication level for a specific operation, and a value below the client's default is raised back to it.
...
| Does the user need OTP? | Target ACR level, which is the higher of `acr_values` and the client default, plus the user's OTP enrollment status and OTP configuration changes |
