...
That has two exceptions: a client that [registered itself](/concepts/clients/#dynamic-client-registration-rfc-7591) never receives an error redirect, silent requests included, and no client receives one at an address it no longer has registered. Goiabada shows the user a page naming the address the client asked to send them to, and the request stops there. See [Declining](#declining).
</Aside>
...
| The redirect would be withheld anyway (a self-registered client, or a `redirect_uri` that cannot name a host or is no longer registered) | The refusal page is shown immediately, with no login |
...
If any check fails, the error is redirected to the client and no UI is shown. Two things stop that: a client that [registered itself](/concepts/clients/#dynamic-client-registration-rfc-7591) never receives an error redirect, and neither does one whose `redirect_uri` is no longer registered on it. The redirect is withheld, so the silent request gets no error back and simply never returns. See [Prompt parameter](/concepts/prompt-parameter/) for what a client should do about that.
...
**How the target ACR is determined:** the target is whichever is higher, the first `acr_values` entry Goiabada recognises or the client's default ACR level.
...
| Client | What happens |
|--------|--------------|
| Created by an administrator | The browser is redirected to the client's redirect URI with `error=access_denied`, and the client shows its own message. If that URI has since been removed from the client, the page below is shown instead |
...
|-----------|----------------|
| `acr_values` names at least one recognised value | The higher of the first recognised value and the client's default ACR level |
| `acr_values` not provided, or none recognised | Client's default ACR level |
...
| Does the user need OTP? | Target ACR level (`acr_values`, client default), user's OTP enrollment status, OTP configuration changes |
