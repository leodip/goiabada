...
An application renewing quietly in the background with [`prompt=none`](/concepts/prompt-parameter/) hits the same check and is told rather than shown: it gets a `login_required` error back, never a login page, so it can decide when to ask the person to sign in again. A client that [registered itself](/concepts/clients/#dynamic-client-registration-rfc-7591) is the exception and gets nothing back at all, because Goiabada withholds its error redirects; a silent request that does not return is the signal to send the person through an ordinary sign-in.
...
Earlier versions of Goiabada did not make this comparison, so a handover on a shared browser could leave a sign-in tied to the previous person's session. If you are upgrading, a few grants of that kind may already be out there.
