...
Treat a silent request that does not return as "send the user through an ordinary authorization". Do that once, without `prompt=none`, and silent renewal works from then on for the scopes they consented to. Clients an administrator registered are unaffected.
</Aside>
