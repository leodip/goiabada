...
<Aside type="tip">
`prompt=none` is most commonly paired with [`id_token_hint`](/concepts/id-token-hint/) for silent token renewal in single page apps. The hint pins the request to a specific user, so a session belonging to a different user won't satisfy the silent flow.
</Aside>
...
<Aside type="caution">
**A self-registered client gets no error back at all.** A client that registered itself through [dynamic client registration](/concepts/clients/#dynamic-client-registration-rfc-7591) requires consent by default, so its first silent request for a user cannot be completed. None of the errors above are delivered for it: an error response is a redirect to an address nobody here has checked, and a `prompt=none` request is the cheapest way to ask for one, so Goiabada [withholds the redirect](/concepts/clients/#dynamic-client-registration-rfc-7591) instead. The request simply does not come back.

Treat a silent request that does not return as "send the user through an ordinary authorization". Do that once, without `prompt=none`, and silent renewal works from then on for the scopes they consented to. Clients an administrator registered are unaffected and receive every error in the table above.
</Aside>
...
Ending it is a full termination rather than a plain delete: the refresh tokens and the unredeemed authorization codes that came from that session are revoked with it. The browser has changed hands, so the grants that browser was holding are now in someone else's reach. Sessions the previous user has on other devices are untouched. See [User sessions](/concepts/user-sessions/).

<Aside type="tip">
When the **same** user re-authenticates and their session is still valid, it is not invalidated: other clients using it will continue to work, and only the current authorization request requires fresh authentication. A session that is no longer valid for this request is replaced by a new one instead, as the table above describes, so that assurance does not apply to it.
</Aside>
