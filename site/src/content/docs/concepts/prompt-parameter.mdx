...
<Aside>
A client that registered itself through [dynamic client registration](/concepts/clients/#dynamic-client-registration-rfc-7591) requires consent by default, so its first silent request for a user returns `consent_required`. Send the user through an ordinary authorization once, without `prompt=none`, and silent renewal works from then on for the scopes they consented to.
</Aside>
