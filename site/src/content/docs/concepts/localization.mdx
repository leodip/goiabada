...
Goiabada localizes all user-facing text: login and consent screens, account self-service, the admin console, validation and error messages, and emails. Country, timezone, and phone-country dropdowns — and the language picker itself — are shown in the active language using Unicode CLDR data.
...
Country and language names, along with most timezone labels, are resolved from CLDR at runtime (a small embedded timezone dataset provides curated labels and fallbacks), so adding a language is essentially **one translation file** — no code changes and no rebuild.
...
<Aside type="tip">
Set `GOIABADA_I18N_OVERRIDES_DIR` on **both** the auth server and the admin console so the language is available everywhere. Only the `catalogs/` directory is honored — reference data (country, timezone, and phone-country names) comes from CLDR plus built-in data and is **not** runtime-overridable.
</Aside>
