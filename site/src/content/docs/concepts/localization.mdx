...
A `ui_locales` value on the authorize request is preserved across the whole login flow (password → OTP → consent). It also applies to the page `/auth/authorize` renders when it refuses a request outright — a `client_id` it cannot resolve, or a `redirect_uri` it will not send anyone to — since that page is shown instead of a redirect and never reaches the client.
