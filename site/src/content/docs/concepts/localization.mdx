...
Reference data (country, timezone, and language names) is generated from CLDR at runtime, so adding a language is essentially **one translation file** — no code changes and no rebuild.
...
$GOIABADA_I18N_OVERRIDES_DIR/
  catalogs/
    active.<locale>.toml
  reference/                    # optional — CLDR covers these automatically
    <locale>/
      countries.toml
      timezones.toml
...
<Aside type="tip">
Set `GOIABADA_I18N_OVERRIDES_DIR` on **both** the auth server and the admin console so the language is available everywhere. The `reference/` bundles are optional — provide them only to override a specific country, timezone, or phone-country label.
</Aside>
