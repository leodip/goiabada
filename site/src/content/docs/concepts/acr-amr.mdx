...
When a user enables or disables OTP, every one of their active sessions owes a level 2 re-authentication, not just the session the change was made from. The prompt appears the next time a session reaches a `level2` client, and it stays owed until it is answered, so a login abandoned at the OTP screen is prompted again on the next attempt. A `prompt=none` request from a session that still owes the prompt is refused with `interaction_required`.
