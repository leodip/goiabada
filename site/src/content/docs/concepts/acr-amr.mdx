...
The client's default ACR level is a floor, not just a starting point. A request asking for a level
below it is raised back to it, so `acr_values=urn:goiabada:level1` against a client configured for
`level2_mandatory` still requires the OTP. The parameter can only ask for a stronger
authentication than the client is configured for, never a weaker one.

<Aside type="tip">
If you request multiple ACR values separated by spaces, Goiabada uses the first one it recognises,
ignoring any it does not, and then raises that to the client's default ACR level if it is below it.
If it recognises none of the values you send, the client's default ACR level applies, exactly as if
the parameter had been left out. The three values it recognises are `urn:goiabada:level1`,
`urn:goiabada:level2_optional` and `urn:goiabada:level2_mandatory`, matched in full.
</Aside>

<Aside type="caution">
Because the returned `acr` can be higher than the one you asked for, compare it with what your
application requires rather than with what it requested. An exact string match against the requested
value will not hold when the client's configured level is higher.
</Aside>
...
// amr is optional in OpenID Connect, so check it is there before reading it.
...
|-----------|----------------|-------------------------------|
| `acr_values` | Authentication strength | Session ACR is lower than the level required, which is the higher of the requested value and the client's default |
