...
If `acr_values` is not specified, Goiabada uses the client's default ACR level. A request asking for
a level **below** that default is raised back to it, so `acr_values=urn:goiabada:level1` against a
client configured for `level2_mandatory` still requires OTP.

<Aside type="tip">
If you request multiple ACR values separated by spaces, Goiabada uses the first one it recognises and
ignores the rest. Values are matched in full against the [three ACR levels](#what-is-acr), so a short
form like `level1` is not recognised; if none of the values you send is, the client's default applies
...
The session must belong to the user who just authenticated. If the browser still holds a session for a different user, that session is treated as no session at all: it contributes no ACR level, so the requested ACR alone decides whether a second factor is needed.
...
// Check the authentication level. acr can come back higher than you asked for,
// so compare it against what this operation requires, not against acr_values.
...
if (Array.isArray(idToken.amr) && idToken.amr.includes('otp')) {
...
When a user enables or disables OTP, Goiabada advances a counter on that user's record. Every session carries the value of that counter as it stood when the session last satisfied the level 2 question, so once the counter moves, all of the user's sessions are behind it at once, not just the one the change was made from.

The next time any of those sessions reaches a `level2` client, the user is prompted to re-authenticate at level 2. The session catches up only once that prompt has been answered, so a login abandoned at the OTP screen still owes the prompt on the next attempt. A `prompt=none` request against a session that is behind is refused with `interaction_required`.
...
|-----------|----------------|-------------------------------|
| `acr_values` | Authentication strength | Session ACR is lower than the level required |
...
**Example:** A user has an active `level2_mandatory` session that started 2 hours ago. A request comes in with `acr_values=urn:goiabada:level1` and `max_age=3600` (1 hour):
