...
<Aside type="caution">
**If you build a self-registered client:** it receives no error response in that case. There is no redirect, so an authorization the user declines or the server refuses simply never completes, and your client should treat an authorization that does not return within a reasonable time as abandoned rather than waiting indefinitely. A `prompt=none` request is the exception and still receives its error by redirect, because [silent authentication](/concepts/prompt-parameter/) must not display anything to the user.
