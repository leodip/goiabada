...
<Aside type="caution">
**A self-registered client can be missing its badge.** Goiabada recognises one by the `dcr_` prefix on its identifier, so a client that was renamed back when identifiers could still be changed does not carry it. Such a client is not marked, not badged, and consent is off for it. Look for it in the client list and tick **Consent required** by hand.
</Aside>
