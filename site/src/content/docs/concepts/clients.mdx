...
The one exception is a field that names a response parameter. If your registered URI carries `state`, `code`, `error` or `error_description`, the server's own value replaces it, because the response may only contain each of those once. So a client that registers `https://example.com/callback?state=fixed` will not see `state=fixed` in the callback: it sees the `state` it sent on the authorization request. Register a different name if you need a fixed value to survive.
