...
A self-registered client's **identifier cannot be changed**, in the same way the [system-level client](#system-level-client)'s cannot. Every other setting can, including **Consent required**. The identifier is how a client is known to everything that has already been told about it, and how it is recognised in a log; a self-registered one is generated rather than chosen, so keeping it fixed costs nothing and means the name in your client list is the name the client actually authenticates under.
...
This holds for [silent authentication](/concepts/prompt-parameter/) too. A `prompt=none` request that fails is the cheapest version of the same attack, because it needs no session and nothing from the user at all, so a self-registered client's silent request that cannot be completed gets no redirect either. The page renders where a browser would show it, and inside the hidden iframe silent renewal normally runs in, nothing is displayed to anyone.

<Aside type="caution">
**If you build a self-registered client:** it receives no error response in any of those cases. There is no redirect, so an authorization the user declines or the server refuses simply never completes, and your client should treat an authorization that does not return within a reasonable time as abandoned rather than waiting indefinitely. That applies to `prompt=none`: a silent request that cannot be completed times out rather than returning `consent_required`, so send the user through one ordinary authorization when a silent one does not come back.

An administrator who has reviewed your client can give it a display name, which changes what the page says, but the redirect is withheld either way. If your integration needs the error response, it needs a client an administrator registered.
</Aside>

<Aside type="caution">
**For operators upgrading:** this changes the behaviour of clients that are already registered. The migration turns consent on for every existing self-registered client, so a client relying on silent renewal with `prompt=none` stops renewing until its user has been through the consent screen once. That is a visible change and it is not a graceful one: the silent request does not come back with `consent_required`, it does not come back at all, so an integration that has been renewing tokens silently needs one interactive authorization per user before it renews silently again.
