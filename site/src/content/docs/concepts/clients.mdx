...
<Aside type="caution">
**A self-registered client can be missing its badge.** One that registered itself before Goiabada started marking them, and was renamed back when that was still possible, carries no mark: no badge, and the refusal behaviour above does not apply to it. Nothing in the client list distinguishes it from a client an administrator created, so if your deployment is old enough to have one, check that **Consent required** is set the way you want across your clients.
</Aside>
