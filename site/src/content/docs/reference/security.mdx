...
- **Rate limiting** - Configurable rate limiting on sensitive endpoints (login, OTP verification, password reset, self-registration, email verification, password and OTP changes)
...
- **No stored copies of the rendered pages or the API responses** - Every page Goiabada renders is sent with `Cache-Control: no-store` and `Pragma: no-cache`, the login form, the one-time code prompt, the page that shows a new authenticator's setup key, and the consent screen among them. Every response from an endpoint under `/api/v1/admin` or `/api/v1/account` carries the same pair, refusals from the token and scope checks included, because two of them hand out a credential outright: a new authenticator's setup key, and a client's secret, decrypted. A few responses under those paths are answered before an endpoint is picked, a CORS preflight and a small set of internal errors among them, and those carry neither field; none of them carries a credential. A bearer token is not protection enough on its own, since a private cache may still keep a copy of a response it authenticated. That binds HTTP caches, so a proxy, a CDN or the browser's own disk cache must not keep a copy and must not answer anyone else with one. It does not bind the browser's history or its back/forward cache, which [RFC 9111](https://www.rfc-editor.org/rfc/rfc9111.html#section-6) leaves free to redisplay a page it has already shown, so this is not a promise that nothing is retained on the machine. See [caching](/integration/rest-api/#caching)
...
- **No open redirection for self-registered clients** - Authorization errors are normally delivered by redirecting the browser to the client's registered address. For a client that [registered itself](/concepts/clients/#dynamic-client-registration-rfc-7591), Goiabada shows the user a page naming that address instead. Anyone can register a client and choose where it points, so without this a refused authorization is a way to send someone's browser to an unvetted site. Clients an administrator created are unaffected
- **No error redirect before the user has authenticated** - The bullet above turns on where the redirect address came from. This one turns on who is at the browser, and it applies to every client, administrator-created ones included. A request that fails validation while nobody is signed in does not produce a redirect: the user is sent to the login page, and the error reaches the client afterwards, unchanged. Otherwise a single link carrying one bad parameter turns the authorization server into a redirector to any address a client has registered, with the victim never having interacted with anything. A browser that already holds a session is answered at once, and so is a `prompt=none` request, which must never show a page. [RFC 9700](https://www.rfc-editor.org/rfc/rfc9700.html) 4.11.2 requires this. See [Deferred error redirects](/concepts/authorization-lifecycle/#deferred-error-redirects)
...
- **HTTP request records go to the application log on stderr** - They are structured attributes on the same stream as every other message the server writes, so there is one log to collect instead of two. **If your deployment captures only stdout, you will not see request records at all until you capture stderr too.** Silence there looks like an absence of traffic rather than a stream nobody is reading, so it is worth checking. The `request_id`, `method` and `ip` attributes are clipped at 128 bytes with the byte count in the marker, so a very long proxy-supplied request id shows up shortened rather than missing
