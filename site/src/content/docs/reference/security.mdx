...
- **The consent screen is also tied to the sign-in it belongs to** - Separate from CSRF, and for a different reason. A browser has one sign-in request in progress at a time, so starting a second one replaces the first. The consent screen carries a hidden value naming the request it was rendered for, and submitting it after another request has replaced that one is refused with a "this request is no longer active" page. The request you are actually on is left untouched and stays completable in its own tab. This applies to the consent screen only; the admin console forms listed above are not part of a sign-in request and carry no such value
