...
- **No open redirection for self-registered clients** - An error response is normally delivered by redirecting the browser to the client's registered address. For a client that registered itself through [dynamic client registration](/concepts/clients/#dynamic-client-registration-rfc-7591), Goiabada shows the user a page naming that address instead, and the request ends there. Anyone can register a client and choose where it points, so without this a refused authorization is a way to send someone's browser to an unvetted site, whether they refused it themselves or the server refused it for them. Clients an administrator created are unaffected. Silent authentication with `prompt=none` is covered as well: it is the version of the attack that needs no session and nothing from the user, so a self-registered client's silent request that cannot be completed returns no error either, and the page renders where a browser would show it
