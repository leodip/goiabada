...
- **Credentials are only read from the request body** - Every form that carries a credential is submitted by `POST`, and the value is read from the request body alone. A password, TOTP code, TOTP secret, email verification code, client secret or SMTP password placed in the URL query instead is not seen at all: the request is answered exactly as though that field had been left blank, so a value in a URL never authenticates, never authorizes and is never stored. At the login and 2FA prompts, password reset, self-registration and the password change page that means the request is refused with the same missing-field error a blank submission gets, and the value in the URL is never compared against anything. Each handler simply takes the path it already takes for that field being blank, which in some of these forms looks the account up first, exactly as it does when the field is genuinely left empty. This holds for every form in both the login pages and the admin console. It matters because a value in a URL escapes the server: it reaches the browser's address bar and history, the `Referer` of anything the page loads, and the access log of every proxy, gateway and CDN in front of your deployment, none of which Goiabada controls. The [redaction](#logging) below is the narrower and separate guarantee, about Goiabada's own log
