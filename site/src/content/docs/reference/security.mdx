...
- **No token and no cookie** - There is no hidden CSRF token in the forms and no CSRF cookie. The origin is the whole check, so there is nothing to expire and nothing to configure
- **The sign-in forms are also tied to the sign-in they belong to** - Separate from CSRF, and for a different reason. A browser has one sign-in request in progress at a time, so starting a second one replaces the first. The login form, the OTP prompt and the consent screen each carry a hidden value naming the request they were rendered for, and submitting one after another request has replaced that one is refused with a "this request is no longer active" page. The request you are actually on is left untouched and stays completable in its own tab. This applies to those three screens only; the admin console forms listed above are not part of a sign-in request and carry no such value
