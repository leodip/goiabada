...
**Secrets still encrypted with a key held in the database.** At startup, if the auth server finds secrets encrypted with a key stored in the database rather than with `GOIABADA_AES_ENCRYPTION_KEY`, it automatically re-encrypts every stored secret and signing key to the env key and clears the old one. This runs in a single transaction and retries safely if interrupted; no extra configuration is needed.
