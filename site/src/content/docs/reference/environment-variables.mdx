...
| Endpoint | Limit | Key |
|----------|-------|-----|
| `POST /auth/pwd` (password login) | 15 req/min | email (lowercased) |
| `POST /auth/pwd` (password login) | 30 req/min | IP address (IPv6 grouped by /64) |
| `POST /auth/otp` (OTP verification) | 10 req/min | user ID |
| `POST /forgot-password` | 5 req/5min | email (lowercased) |
| `POST /forgot-password` | 20 req/5min | IP address (IPv6 grouped by /64) |
| `GET` and `POST /reset-password` | 30 req/5min | IP address (IPv6 grouped by /64) |
| `GET /account/activate` | 20 req/5min | IP address (IPv6 grouped by /64) |
| `POST /connect/register` (DCR) | 10 req/min | IP address (IPv6 grouped by /64) |
| `POST /auth/token` (ROPC grant) | 5 req/min | client_id + username + IP (IPv6 grouped by /64) |
