...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td style="white-space: nowrap;"><code>scope</code></td>
      <td>For <code>client_credentials</code>: required, one or more <code>resource:permission</code> scopes. For <code>refresh_token</code>: optional, to restrict the original scope. A value the original grant does not hold is refused with <code>invalid_scope</code>.</td>
    </tr>
    <tr>
...
<Aside type="caution">
Use a JOSE/JWE library for your platform (the examples below do).
</Aside>
...
The endpoint takes an access token issued for a user whose scope includes `openid`.
...
Request with scopes `openid`, `profile` and `email`:
...
- **401 Unauthorized** - `invalid_token`: the user account is disabled or no longer exists
- **401 Unauthorized** - `invalid_token`: the session the access token was issued in has ended
- **403 Forbidden** - Access token lacks the `openid` scope (a client credentials token never has it)
