...
|-----|----------------|---------|
| **Admin API**<br/>`/api/v1/admin` | `adminconsole:manage` | Full administrative control |
| **Account API**<br/>`/api/v1/account` | `adminconsole:manage-account` | Self-service account management |
...
2. For **Admin API** access:
   - Select the `adminconsole` resource
   - Assign the `manage` permission
   - This gives full access to all Admin API endpoints
...
<Aside type="note">
The **Admin API** scope (`adminconsole:manage`) provides full administrative control over your Goiabada instance. Only assign this to trusted applications. For user self-service operations, use the **Account API** scope (`adminconsole:manage-account`) instead.
</Aside>
