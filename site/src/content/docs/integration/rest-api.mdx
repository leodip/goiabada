...
Every response from an endpoint in either API group is sent with `Cache-Control: no-store` and `Pragma: no-cache`, refusals from the token and scope checks included. Do not cache these responses, and if your HTTP client caches `GET` requests by default, make sure it honours those directives. Two endpoints return a credential in the response body: `GET /api/v1/admin/clients/{id}` returns the client secret decrypted, and `GET /api/v1/account/otp/enrollment` returns a new authenticator's setup key. The bearer token on the request does not make a cached copy safe.

A few responses under these paths are answered by the server before it picks an endpoint, and those carry neither header: a CORS preflight, and the errors Goiabada returns when it cannot read its own settings or your session. None of them carries a credential or a response body worth keeping.
...
**Field notes:**
- `auditLogsInConsoleEnabled`: Log audit events to the console via structured logging (slog)
...
```
GET /api/v1/admin/audit-logs?page=1&pageSize=20&auditEvent=user_login
```
...
Issues the enrollment and returns the QR code and secret key to set up an authenticator with:
...
The server records the secret it issued, and enables only that one. While an enrollment is pending
this call is idempotent: it returns the same secret and the same QR code, so a user who reloads your
enrollment page keeps the code they have already scanned. A pending enrollment lasts 15 minutes,
after which the next call issues a new one.
...
Call the enrollment endpoint first. The code is verified against the secret the server issued there,
and that secret is what gets enrolled.

<Aside type="caution">
Do not send a `secretKey` with this request. It is refused with `400 SECRET_KEY_NOT_ACCEPTED` rather
than ignored, because a request carrying one is asking to choose which authenticator is installed,
which the server no longer permits. If no enrollment is pending, or the one started has expired, the
request is refused with `400 OTP_ENROLLMENT_NOT_PENDING`.
</Aside>
