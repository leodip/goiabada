...
All endpoints return errors in a consistent, flat format:
...
- `error_code` — a stable identifier for the failure. Either an `UPPER_SNAKE` category (see below) or, for localizable validation errors, a translation-catalog key (e.g. `validator.address.locality_too_long`).
- `error_args` — optional parameters for that message, present only for parameterized errors.
- `error_description` — human-readable text, localized to the request's language when applicable, English otherwise.

Route on the **HTTP status code**, not the body.

Common `error_code` categories:
...
A save that replaces a whole list also sends the list as last read, in its `expected` field. If the stored list differs, the save answers `409` with error code `CONCURRENT_UPDATE` and changes nothing: read it again and retry.

<Aside type="caution">
This flat shape replaced the older nested `{ "error": { "code", "message" } }` envelope. Protocol endpoints (`/auth/token`, `/auth/authorize`, `/connect/register`, `/userinfo`) keep their RFC-defined error format and are unaffected.
</Aside>
...
Returns the user's own fields: profile, email, phone, address and OTP state. Groups, permissions and attributes are not included; each is served by its own endpoint, documented below: `GET /api/v1/admin/users/{id}/groups`, `/permissions` and `/attributes`.
...
| <span style="white-space: nowrap;">`setPasswordType`</span> | string | No | `"now"` to set password immediately, `"email"` to send setup email. Defaults to `"now"` when absent; any other value is rejected with 400 |
| <span style="white-space: nowrap;">`password`</span> | string | Conditional | Required unless a setup email will be sent, so: required whenever `setPasswordType` is not `"email"`, and required whatever it says on a deployment with no SMTP configured |
...
The verification code is 8 characters (4 letters + 4 numbers) and expires after 5 minutes.
...
Returns the user's active sessions with their device, IP address and the identifiers of the clients they authorized, so an expired session is not in the list.
...
```json
{
  ...
  "expectedPermissions": [{ "id": 5, "permissionIdentifier": "write", "description": "Write access" }]
}
```
...
```json
{
  ...
  "expectedRedirectURIs": ["https://myapp.com/callback"]
}
```
...
Returns the client's active sessions alongside a normalized `users` array holding the name and email of each owner, so a session's `userId` is resolved from the same response rather than read back one user at a time.

#### Client logo

##### Get client logo status

```
GET /api/v1/admin/clients/{id}/logo
```

Returns whether the client has a logo and the URL to retrieve it.
...
{
  "hasLogo": true,
  "logoUrl": "https://auth.example.com/client/logo/my-web-app"
}
```
...
Upload a logo for a client. Send as `multipart/form-data` with field name `picture`.
...
{
  "success": true,
  "pictureUrl": "https://auth.example.com/client/logo/my-web-app"
}
```
...
```
DELETE /api/v1/admin/clients/{id}/logo
```

Removes the client's logo.
...
<Aside type="note">
System-level clients (such as `admin-console-client`) cannot be deleted. Attempting to do so will return a validation error.
</Aside>
...
PUT /api/v1/admin/settings/audit-logs
```
...
**Field notes:**
- `auditLogsInConsoleEnabled`: Write each audit event to the application log as one record, with the event name and its details as fields
- `auditLogsInDatabaseEnabled`: Persist audit events to the `audit_logs` database table
- `auditLogRetentionDays`: Number of days to retain audit logs in the database. Use `0` for infinite retention. Maximum `3650` (10 years). A background worker periodically deletes logs older than this value.

#### Audit logs (viewer)

```
GET /api/v1/admin/audit-logs?page=1&size=20&auditEvent=auth_success_pwd&requestId=host/Ppg6bHPK5f-000012
```

Returns paginated audit log entries from the database. All query parameters are optional.

**Field notes:**
- `auditEvent`: Return only entries carrying this event identifier, matched exactly. The accepted values are the catalog below, and a value outside it returns an empty page rather than an error
- `requestId`: Return only entries carrying this request id, matched exactly
  ...
  "auditLogs": [
    {
      ...
      "auditEvent": "auth_success_pwd",
      "details": "{\"userId\":\"123\",\"ip\":\"192.168.1.1\"}",
      "requestId": "host/Ppg6bHPK5f-000012"
    }
  ],
  "total": 150,
  "page": 1,
...
Returns the catalog of audit event identifiers this server can write, which is the set the `auditEvent` filter accepts. The list is fixed at build time rather than derived from the rows present, so it carries events that have not occurred on this deployment yet. The admin console's audit log viewer fills its filter dropdown from this endpoint.

```json
{
  "auditEventTypes": [
    "activated_account",
    "added_group_attribute",
    "added_group_permission",
    "added_user_attribute"
  ]
}
```

The response carries the whole catalog, sorted by identifier; the sample above is its first four entries.
...
Repeat calls return the same secret and QR code while the enrollment is pending (15 minutes).
...
Call the enrollment endpoint first: the code is verified against the secret it issued, and that secret is the one enrolled. Sending a `secretKey` is refused with `400 SECRET_KEY_NOT_ACCEPTED`; no pending enrollment gives `400 OTP_ENROLLMENT_NOT_PENDING`.
...
Returns the caller's active sessions with their device, IP address and the identifiers of the clients they authorized, so an expired session is not in the list. The account, user and client session list endpoints mark the session the calling token was issued through with `isCurrent`, which is `false` throughout for a token carrying no `sid` claim, such as one obtained through the client credentials grant.
...
| `responseMode` | string | No | `"form_post"` or `"redirect"` (the default) |

**Response, with `responseMode` set to `"redirect"` or omitted:**
...
**Response, with `responseMode` set to `"form_post"`:**
...
`method` is always `"POST"` and `endpoint` carries no query string of its own. Render `params` as the
hidden inputs of a form your page submits itself to `endpoint`, rather than as a query string:
keeping the `id_token_hint` out of a URL is the point of this mode, since a URL reaches the address
bar, the browser history and the access log of every proxy on the way.
