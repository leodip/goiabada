...
- [ ] **Secure cookies** - Automatic when the base URLs are `https://` (the `Secure` flag is derived from the base URL scheme; there is no separate setting)
...
- [ ] **Trusted proxies set for multi-hop / CDN** - If more than one proxy sits in front of Goiabada (e.g. a CDN plus Nginx), set `GOIABADA_AUTHSERVER_TRUSTED_PROXIES` (and the admin-console equivalent) to your proxy IPs/CIDRs so the client IP cannot be spoofed. See [Reverse proxy: client IP resolution](/production-deployment/reverse-proxy/#client-ip-resolution-and-spoofing-protection)
...
- [ ] **Audit log retention reviewed** - Confirm the **Audit log settings** page reflects your compliance and storage requirements (defaults: console and database both enabled, 180 day retention). See [Audit log](/concepts/audit-log/).
