...
2. Set the deployment strategy so the old pods go away before the new ones start:
   ```yaml
   spec:
     strategy:
       type: Recreate
   ```
   Or scale to zero by hand. `kubectl scale` returns as soon as the API accepts the change, not when the pods are gone, so wait for them to be deleted before you go any further. Applying while an old pod is still terminating starts the new version alongside it, which is the state this whole section exists to avoid:
   ```bash
