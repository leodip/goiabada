...
7. **Ceiling comments**: a simplification that ships with a known limit carries a `ceiling:` comment naming the limit, what it costs, and the event that would make it worth revisiting (`Revisit when ...`, or `until`), then the issue number. A global lock where per-account locks would scale, a linear scan over a list that is currently short, a heuristic standing in for a rule. The trigger is what makes it a decision rather than a TODO: without one, nobody can tell whether the shortcut was weighed or merely postponed, and it becomes permanent by default. Not every simplification, only one a later reader could mistake for the final design. Existing `TODO`s are left as they are: a TODO is a wish, a ceiling is a decision
