...
<tr><td colspan="2">File path where bootstrap credentials will be automatically written during first-time database initialization. This is useful for automated deployments (Docker, Kubernetes, CI/CD pipelines) where you need to capture auto-generated credentials.
<br /><br />
<strong>What gets written:</strong>
<ul>
<li>Admin console OAuth client ID and secret (<code>GOIABADA_ADMINCONSOLE_OAUTH_CLIENT_ID</code>, <code>GOIABADA_ADMINCONSOLE_OAUTH_CLIENT_SECRET</code>)</li>
<li>Auth server session authentication and encryption keys (hex-encoded)</li>
<li>Admin console session authentication and encryption keys (hex-encoded)</li>
</ul>
The file is created with <code>0600</code> permissions (read/write for owner only) for security. Parent directories are automatically created with <code>0700</code> permissions if they don't exist. The output is in standard <code>.env</code> format (one variable per line).
<br /><br />
<strong>How the admin console uses it:</strong>
<br />
In a multi-container setup (Docker Compose, Kubernetes), the auth server and admin console run as separate containers. The workflow is:
<ol>
<li><strong>Auth server generates</strong> - On first startup, detects empty database, generates credentials, writes to the bootstrap file path you specified</li>
<li><strong>You configure admin console</strong> - In your Docker Compose/Kubernetes config, you manually specify the bootstrap file location using <code>env_file</code> (Docker) or <code>envFrom</code> (Kubernetes)</li>
<li><strong>Admin console loads credentials</strong> - Docker/Kubernetes reads the file and injects the variables as environment variables into the admin console container</li>
<li><strong>Admin console authenticates</strong> - Uses the loaded OAuth credentials to communicate with the auth server</li>
</ol>
<strong>Docker Compose example:</strong>
<pre><code># Auth server (generates credentials on first run)
goiabada-authserver:
  volumes:
    - ./bootstrap:/bootstrap  # Mount shared volume
  environment:
    - GOIABADA_AUTHSERVER_BOOTSTRAP_ENV_OUTFILE=/bootstrap/credentials.env

# Admin console (you configure it to load the generated file)
goiabada-adminconsole:
  depends_on:
    - goiabada-authserver
  volumes:
    - ./bootstrap:/bootstrap  # Mount same shared volume
  env_file:
    - ./bootstrap/credentials.env  # File generated by the auth server
</code></pre>
<strong>Important:</strong> The admin console doesn't automatically discover the bootstrap file. You must explicitly configure the <code>env_file</code> path in your deployment configuration to match the path where the auth server writes the file. Both containers need access to the same shared volume or file path.
<br /><br />
<strong>Note:</strong> This only runs during the initial database seeding (when the database is empty). If you restart the containers without wiping the database, the file will not be regenerated. Default: <em>(empty)</em></td></tr>
</table>
