...
## Option 5: Kubernetes cluster deployment

**✅ Suitable for production**

Deploying Goiabada on Kubernetes follows the same principles as Docker Compose, but adapted to Kubernetes resources. The key challenge is handling Goiabada's two-phase bootstrap process: first, run the auth server as a Kubernetes Job to generate OAuth credentials and session keys, which writes them to a PersistentVolume. Then extract these credentials from the volume, create Kubernetes Secrets from them, and deploy the auth server and admin console Deployments referencing those secrets.

Configure an Ingress resource with cert-manager for automatic TLS certificates, routing traffic to two Services (auth server on port 9090, admin console on 9091). Ensure you set `GOIABADA_AUTHSERVER_TRUST_PROXY_HEADERS=true` and `GOIABADA_AUTHSERVER_SET_COOKIE_SECURE=true` (and equivalent admin console variables) since you'll be behind an Ingress controller.

The standard Kubernetes resources you'll need are: a dedicated Namespace, Secrets (database credentials and bootstrap-generated keys), ConfigMaps (for non-sensitive configuration like URLs and app name), two Deployments (one for auth server, one for admin console using the `leodip/goiabada:authserver-latest` and `adminconsole-latest` images), two ClusterIP Services exposing ports 9090 and 9091, and an Ingress with TLS configured for both domains. For high availability, run multiple replicas with appropriate resource limits and pod anti-affinity rules.
