...
func (s *Server) Start(settings *models.Settings) {
	...
	slog.Info("host: " + host)
	slog.Info("port: " + port)
	slog.Info("base url: " + config.AdminConsoleBaseUrl)

	if config.IsHttpsEnabled() {
		if !strings.HasPrefix(config.AdminConsoleBaseUrl, "https://") {
		...
		// non-TLS mode
		if !strings.HasPrefix(config.AdminConsoleBaseUrl, "http://") {
...
func (s *Server) initMiddleware(settings *models.Settings) {
	...
	if config.RateLimiterEnabled {
		slog.Info(fmt.Sprintf("http rate limiter enabled. max requests: %v, window size in seconds: %v", config.RateLimiterMaxRequests, config.RateLimiterWindowSizeInSeconds))
		s.router.Use(MiddlewareRateLimiter(s.sessionStore, config.RateLimiterMaxRequests, config.RateLimiterWindowSizeInSeconds))
	} else {
