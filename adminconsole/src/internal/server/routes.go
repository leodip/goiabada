...
import (
	...
	"github.com/leodip/goiabada/adminconsole/internal/handlers/handlerhelpers"
	"github.com/leodip/goiabada/adminconsole/internal/inputsanitizer"
	"github.com/leodip/goiabada/adminconsole/internal/oauth"
	"github.com/leodip/goiabada/adminconsole/internal/otp"
...
func (s *Server) initRoutes() {
	...
	middlewareJwt := NewMiddlewareJwt(s.sessionStore, tokenParser, s.database, authHelper)
	...
	s.router.Route("/account", func(r chi.Router) {
		r.With(jwtSessionHandler).With(requiresAccountScope).Get("/", func(w http.ResponseWriter, r *http.Request) {
			http.Redirect(w, r, config.AuthServerBaseUrl+"/account/profile", http.StatusFound)
		})
