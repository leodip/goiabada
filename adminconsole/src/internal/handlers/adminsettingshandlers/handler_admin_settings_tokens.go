...
func HandleAdminSettingsTokensPost(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		audit.Log(constants.AuditUpdatedTokensSettings, map[string]interface{}{
		...
		http.Redirect(w, r, fmt.Sprintf("%v/admin/settings/tokens", config.AdminConsoleBaseUrl), http.StatusFound)
	}
}
