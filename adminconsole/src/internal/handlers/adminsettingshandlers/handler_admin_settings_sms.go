...
func HandleAdminSettingsSMSGet(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(settings.SMSProvider) > 0 && settings.SMSProvider == "twilio" {
			smsConfigDecrypted, err := encryption.DecryptText(settings.SMSConfigEncrypted, settings.AESEncryptionKey)
			if err != nil {
...
func HandleAdminSettingsSMSPost(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case "twilio":
			...
			smsConfigEncrypted, err := encryption.EncryptText(string(smsConfigJson), settings.AESEncryptionKey)
			if err != nil {
				httpHelper.InternalServerError(w, r, err)
				return
			}
		...
		audit.Log(constants.AuditUpdatedSMSSettings, map[string]interface{}{
		...
		http.Redirect(w, r, fmt.Sprintf("%v/admin/settings/sms", config.AdminConsoleBaseUrl), http.StatusFound)
	}
}
