...
func HandleAdminSettingsEmailPost(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		audit.Log(constants.AuditUpdatedSMTPSettings, map[string]interface{}{
		...
		http.Redirect(w, r, fmt.Sprintf("%v/admin/settings/email", config.AdminConsoleBaseUrl), http.StatusFound)
	}
}
...
func HandleAdminSettingsEmailSendTestPost(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		http.Redirect(w, r, fmt.Sprintf("%v/admin/settings/email/send-test-email", config.AdminConsoleBaseUrl), http.StatusFound)
	}
}
