...
func HandleAdminUserAttributesEditPost(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		audit.Log(constants.AuditUpdatedUserAttribute, map[string]interface{}{
			"userId":          user.Id,
			"userAttributeId": attribute.Id,
			"loggedInUser":    authHelper.GetLoggedInSubject(r),
		})

		http.Redirect(w, r, fmt.Sprintf("%v/admin/users/%v/attributes?page=%v&query=%v", config.AdminConsoleBaseUrl, user.Id,
