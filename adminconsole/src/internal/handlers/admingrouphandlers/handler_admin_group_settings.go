...
func HandleAdminGroupSettingsPost(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		audit.Log(constants.AuditUpdatedGroup, map[string]interface{}{
			"groupId":         group.Id,
			"groupIdentifier": group.GroupIdentifier,
		...
		http.Redirect(w, r, fmt.Sprintf("%v/admin/groups/%v/settings", config.AdminConsoleBaseUrl, group.Id), http.StatusFound)
	}
}
