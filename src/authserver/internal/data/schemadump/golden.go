...
// regeneration is caught in the authserver internal tier rather than only on the four database
// jobs (#288).
...
func Encode(g Golden) ([]byte, error) {
	...
	b.WriteString("#\n")
	b.WriteString("# Generated by `go run ./cmd/schemadump` from src/authserver, which regenerates all four\n")
...
// authserver/internal/data/<engine>db/schema.golden, taking over the spot the hand-maintained
...
func GoldenPath(d Dialect) (string, error) {
	...
	return filepath.Join(root, "authserver", "internal", "data", string(d)+"db", "schema.golden"), nil
}
