...
WHERE (dc.parent_object_id = OBJECT_ID(N'[dbo].[settings]') AND c.name IN (N'pkce_required'))
   OR (dc.parent_object_id = OBJECT_ID(N'[dbo].[clients]') AND c.name IN (N'pkce_required'));
