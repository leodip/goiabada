...
WHERE (dc.parent_object_id = OBJECT_ID(N'clients') AND c.name IN (N'include_open_id_connect_claims_in_id_token'))
   OR (dc.parent_object_id = OBJECT_ID(N'settings') AND c.name IN (N'include_open_id_connect_claims_in_id_token'));
