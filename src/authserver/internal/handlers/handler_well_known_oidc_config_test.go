...
func TestHandleWellKnownOIDCConfigGet(t *testing.T) {
	t.Run("Returns correct OIDC configuration", func(t *testing.T) {
		...
		httpHelper.On("EncodeJson", rr, req, mock.AnythingOfType("oidc.WellKnownConfig")).Run(func(args mock.Arguments) {
			...
			assert.Equal(t, config.Get().BaseURL+"/auth/authorize", wellKnownConfig.AuthorizationEndpoint)
			assert.Equal(t, config.Get().BaseURL+"/auth/token", wellKnownConfig.TokenEndpoint)
			assert.Equal(t, config.Get().BaseURL+"/userinfo", wellKnownConfig.UserInfoEndpoint)
			assert.Equal(t, config.Get().BaseURL+"/auth/logout", wellKnownConfig.EndSessionEndpoint)
			assert.Equal(t, config.Get().BaseURL+"/certs", wellKnownConfig.JWKsURI)
