...
func (s *Server) initRoutes() {
	...
	s.router.Route("/auth", func(r chi.Router) {
		...
		r.Post("/pwd", handlers.HandleAuthPwdPost(httpHelper, authHelper, userSessionManager, s.database, auditLogger))
		r.Get("/otp", handlers.HandleAuthOtpGet(httpHelper, s.sessionStore, authHelper, s.database, otpSecretGenerator))
		r.Post("/otp", handlers.HandleAuthOtpPost(httpHelper, s.sessionStore, authHelper, userSessionManager, s.database, auditLogger))
		r.Get("/consent", handlers.HandleConsentGet(httpHelper, authHelper, s.database, s.templateFS, codeIssuer, permissionChecker, auditLogger))
		r.Post("/consent", handlers.HandleConsentPost(httpHelper, authHelper, s.database, s.templateFS, codeIssuer, auditLogger))
	...
	s.router.Route("/account", func(r chi.Router) {
		...
		r.Get("/activate", accounthandlers.HandleAccountActivateGet(httpHelper, s.database, userCreator, auditLogger))
	})
}
