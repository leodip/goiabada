...
func repairCases000034(t *testing.T) []repair000034 {
	...
	cases := []repair000034{
		...
		{"https://dport443.example.com:80:443", "", true,
			"two ports, the last of them the scheme's default. The default-port strip must not run before the authority has been checked for a second colon, or this becomes the live, browser-reachable https://dport443.example.com:80 and the delete then keeps it"},
		{"http://dport80.example.com:8080:80", "", true,
			"the same shape on http, where the strip is three characters rather than four"},
		{"https://dport2.example.com:443:443", "", true,
			"the same default port twice: stripping one leaves https://dport2.example.com:443, which is not canonical either, so an unguarded strip retains a row that is still dead"},
		{"https://tport.example.com:80:443:443", "", true,
			"three ports, which survives one strip with two colons left and is caught by the colon-count clause; it pins that the guard is what does the work in the two-colon cases above"},
