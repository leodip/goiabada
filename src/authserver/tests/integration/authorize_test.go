...
func TestAuthorize_ClientIdIsMissing(t *testing.T) {
	...
	url := config.Get().BaseURL + "/auth/authorize/"
...
func TestAuthorize_ClientDoesNotExist(t *testing.T) {
	url := config.Get().BaseURL + "/auth/authorize/?client_id=does_not_exist"
...
func TestAuthorize_ClientIsDisabled(t *testing.T) {
	...
	url := config.Get().BaseURL + "/auth/authorize/?client_id=test-client-3"
...
func TestAuthorize_RedirectURIIsMissing(t *testing.T) {
	...
	url := config.Get().BaseURL + "/auth/authorize/?client_id=test-client-1"
...
func TestAuthorize_AccetableResponseModes(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		assert.Equal(t, config.Get().BaseURL+"/auth/pwd", redirectLocation.String())
	}
}
...
func TestAuthorize_PermissionNotGrantedToUser(t *testing.T) {
	...
	resp = getPage(t, httpClient, config.Get().BaseURL+"/auth/pwd")
	defer resp.Body.Close()
	...
	resp = getPage(t, httpClient, config.Get().BaseURL+"/auth/consent")
	defer resp.Body.Close()
...
func TestAuthorize_OneLogin_Pwd_Otp_WithFullConsent(t *testing.T) {
	...
	resp = getPage(t, httpClient, config.Get().BaseURL+"/auth/otp")
	defer resp.Body.Close()
