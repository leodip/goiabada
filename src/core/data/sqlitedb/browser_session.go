...
func (d *SQLiteDatabase) GetBrowserSessionByOwnerAndSessionIdHash(tx *sql.Tx, owner, sessionIdHash string,
...
func (d *SQLiteDatabase) UpdateBrowserSessionData(tx *sql.Tx, owner, sessionIdHash, data string,
	now, expiresAt time.Time) (bool, error) {
	return d.CommonDB.UpdateBrowserSessionData(tx, owner, sessionIdHash, data, now, expiresAt)
}

func (d *SQLiteDatabase) TouchBrowserSession(tx *sql.Tx, owner, sessionIdHash string,
	now, expiresAt time.Time) (bool, error) {
	return d.CommonDB.TouchBrowserSession(tx, owner, sessionIdHash, now, expiresAt)
}

func (d *SQLiteDatabase) DeleteBrowserSession(tx *sql.Tx, owner, sessionIdHash string) error {
	return d.CommonDB.DeleteBrowserSession(tx, owner, sessionIdHash)
}

func (d *SQLiteDatabase) DeleteExpiredBrowserSessions(tx *sql.Tx, now time.Time) error {
