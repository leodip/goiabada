...
SELECT datetime('now'), datetime('now'), 'admin-read', 'Read-only access to all admin API endpoints', id
...
INSERT INTO permissions (created_at, updated_at, permission_identifier, description, resource_id)
SELECT datetime('now'), datetime('now'), 'manage-users', 'Manage users, groups, and permissions', id
FROM resources WHERE resource_identifier = 'authserver'
...
INSERT INTO permissions (created_at, updated_at, permission_identifier, description, resource_id)
SELECT datetime('now'), datetime('now'), 'manage-clients', 'Manage OAuth2 clients', id
FROM resources WHERE resource_identifier = 'authserver'
...
INSERT INTO permissions (created_at, updated_at, permission_identifier, description, resource_id)
SELECT datetime('now'), datetime('now'), 'manage-settings', 'Manage system settings and signing keys', id
FROM resources WHERE resource_identifier = 'authserver'
