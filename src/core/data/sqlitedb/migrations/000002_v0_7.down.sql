...
--
-- The three rebuilt tables declare their columns with the SAME TYPE NAMES 000001 used, not
-- with the ones 000002's up chose. SQLite stores the declared type verbatim and reports it
-- back, so `numeric` and `int` are not interchangeable with `INTEGER` here even though all
-- three carry the same affinity and nothing behaves differently: a rolled back database and a
-- fresh install of the release being rolled back to would otherwise disagree in the catalog,
-- which is what #284's golden comparison reads (#268 decision 12).
