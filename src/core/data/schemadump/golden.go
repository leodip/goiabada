...
// columns are not.
func Encode(schema Schema, d Dialect) ([]byte, error) {
	if !d.valid() {
	...
	var b bytes.Buffer
	fmt.Fprintf(&b, "# goiabada schema dump\tversion=%d\tengine=%s\n", goldenVersion, d)
	b.WriteString("#\n")
...
func Parse(b []byte) (Dialect, Schema, error) {
	var (
		dialect Dialect
		haveHdr bool
		schema  Schema
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(text, "# goiabada schema dump") {
			if haveHdr {
				return "", nil, fmt.Errorf("schemadump: golden file line %d: a second header", line)
			}
			d, err := parseHeader(text)
			if err != nil {
				return "", nil, fmt.Errorf("schemadump: golden file line %d: %w", line, err)
			}
			dialect, haveHdr = d, true
			continue
		}
		...
		if !haveHdr {
			return "", nil, fmt.Errorf("schemadump: golden file line %d: a record before the header", line)
		}
	...
	if err := scanner.Err(); err != nil {
		return "", nil, fmt.Errorf("schemadump: reading the golden file: %w", err)
	}
	if !haveHdr {
		return "", nil, fmt.Errorf("schemadump: the golden file carries no header")
	}
	if len(schema) == 0 {
		return "", nil, fmt.Errorf("schemadump: the %s golden file records no tables at all", dialect)
	}
	for _, e := range schema {
		if len(e.Table.Columns) == 0 {
			return "", nil, fmt.Errorf("schemadump: the %s golden file records table %q with no columns", dialect, e.Name)
		}
	}
	return dialect, schema, nil
}

func parseHeader(text string) (Dialect, error) {
	...
	if version != strconv.Itoa(goldenVersion) {
		return "", fmt.Errorf("golden file format version %q, but this build writes and reads version %d", version, goldenVersion)
	}
	d := Dialect(engine)
	if !d.valid() {
		return "", fmt.Errorf("golden file names engine %q, which is none of the four dialects", engine)
	}
	return d, nil
}
