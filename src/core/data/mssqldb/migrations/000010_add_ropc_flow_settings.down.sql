...
WHERE (t.name = N'clients' AND c.name IN (N'resource_owner_password_credentials_enabled'))
   OR (t.name = N'settings' AND c.name IN (N'resource_owner_password_credentials_enabled'));
