...
WHERE (dc.parent_object_id = OBJECT_ID(N'clients') AND c.name IN (N'resource_owner_password_credentials_enabled'))
   OR (dc.parent_object_id = OBJECT_ID(N'settings') AND c.name IN (N'resource_owner_password_credentials_enabled'));
