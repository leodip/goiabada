...
func TestHandleAdminClientPermissionsPost_SendsTheLoadedList(t *testing.T) {
	testCases := []struct {
		name         string
		body         string
		wantWanted   []int64
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			HandleAdminClientPermissionsPost(httpHelper, nil, stub).ServeHTTP(httptest.NewRecorder(), req)
...
func TestHandleAdminClientPermissionsPost_AConflictReachesTheBrowser(t *testing.T) {
	...
	HandleAdminClientPermissionsPost(httpHelper, nil, stub).ServeHTTP(rec, req)
