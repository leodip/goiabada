...
func load() {
	...
	DBType = getEnv("GOIABADA_DB_TYPE", "mysql")
	DBUsername = getEnv("GOIABADA_DB_USERNAME", "root")
	DBPassword = getEnv("GOIABADA_DB_PASSWORD", "")
	DBHost = getEnv("GOIABADA_DB_HOST", "localhost")
	DBPort = getEnvAsInt("GOIABADA_DB_PORT", 3306)
	DBName = getEnv("GOIABADA_DB_NAME", "goiabada")
	DBDSN = getEnv("GOIABADA_DB_DSN", "file::memory:?cache=shared")
