...
func (s *Server) initRoutes() {
	...
	s.router.NotFound(handlers.HandleNotFoundGet(s))
	s.router.Get("/", handlers.HandleIndexGet(s))
	s.router.Get("/unauthorized", handlers.HandleUnauthorizedGet(s))
	s.router.Get("/forgot-password", handlers.HandleForgotPasswordGet(s))
	...
	s.router.With(jwtSessionToContext).Route("/auth", func(r chi.Router) {
		...
		r.Get("/pwd", handlers.HandleAuthPwdGet(s, s, s.database))
		r.Post("/pwd", handlers.HandleAuthPwdPost(s, s, s, s.database, loginManager))
		...
		r.Get("/consent", handlers.HandleConsentGet(s, s, s.database, s.templateFS, codeIssuer, permissionChecker))
		r.Post("/consent", handlers.HandleConsentPost(s, s, s.database, s.templateFS, codeIssuer))
		r.Post("/token", handlers.HandleTokenPost(s, s, s.database, tokenIssuer, tokenValidator))
		r.Post("/callback", handlers.HandleAuthCallbackPost(s, s.sessionStore, s.database, s.tokenParser, tokenIssuer, tokenValidator))
	...
	s.router.With(jwtSessionToContext).With(s.requiresAdminScope).Route("/admin", func(r chi.Router) {
		...
		r.Get("/get-permissions", handlers.HandleAdminGetPermissionsGet(s, s.database))
		...
		r.Get("/clients/generate-new-secret", adminclienthandlers.HandleAdminClientGenerateNewSecretGet(s))
		...
		r.Get("/groups/new", admingrouphandlers.HandleAdminGroupNewGet(s))
		r.Post("/groups/new", admingrouphandlers.HandleAdminGroupNewPost(s, s, s.database, identifierValidator, inputSanitizer))

		r.Get("/users", adminuserhandlers.HandleAdminUsersGet(s, s.database))
		r.Get("/users/{userId}/details", adminuserhandlers.HandleAdminUserDetailsGet(s, s.sessionStore, s.database))
		r.Post("/users/{userId}/details", adminuserhandlers.HandleAdminUserDetailsPost(s, s.sessionStore, s, s.database))
		r.Get("/users/{userId}/profile", adminuserhandlers.HandleAdminUserProfileGet(s, s.sessionStore, s.database))
		...
		r.Get("/users/{userId}/email", adminuserhandlers.HandleAdminUserEmailGet(s, s.sessionStore, s.database))
		...
		r.Get("/users/{userId}/phone", adminuserhandlers.HandleAdminUserPhoneGet(s, s.sessionStore, s.database))
		...
		r.Get("/users/{userId}/address", adminuserhandlers.HandleAdminUserAddressGet(s, s.sessionStore, s.database))
		...
		r.Get("/users/{userId}/authentication", adminuserhandlers.HandleAdminUserAuthenticationGet(s, s.sessionStore, s.database))
		...
		r.Get("/users/{userId}/consents", adminuserhandlers.HandleAdminUserConsentsGet(s, s.sessionStore, s.database))
		r.Post("/users/{userId}/consents", adminuserhandlers.HandleAdminUserConsentsPost(s, s, s.database))
		r.Get("/users/{userId}/sessions", adminuserhandlers.HandleAdminUserSessionsGet(s, s.database))
		r.Post("/users/{userId}/sessions", adminuserhandlers.HandleAdminUserSessionsPost(s, s, s.database))
		r.Get("/users/{userId}/attributes", adminuserhandlers.HandleAdminUserAttributesGet(s, s.database))
		r.Get("/users/{userId}/attributes/add", adminuserhandlers.HandleAdminUserAttributesAddGet(s, s.database))
		r.Post("/users/{userId}/attributes/add", adminuserhandlers.HandleAdminUserAttributesAddPost(s, s, s.database, identifierValidator, inputSanitizer))
		r.Get("/users/{userId}/attributes/edit/{attributeId}", adminuserhandlers.HandleAdminUserAttributesEditGet(s, s.database))
		r.Post("/users/{userId}/attributes/edit/{attributeId}", adminuserhandlers.HandleAdminUserAttributesEditPost(s, s, s.database, identifierValidator, inputSanitizer))
		r.Post("/users/{userId}/attributes/remove/{attributeId}", adminuserhandlers.HandleAdminUserAttributesRemovePost(s, s, s.database))
		r.Get("/users/{userId}/permissions", adminuserhandlers.HandleAdminUserPermissionsGet(s, s.sessionStore, s.database))
		r.Post("/users/{userId}/permissions", adminuserhandlers.HandleAdminUserPermissionsPost(s, s.sessionStore, s, s.database))
		r.Get("/users/{userId}/groups", adminuserhandlers.HandleAdminUserGroupsGet(s, s.sessionStore, s.database))
		r.Post("/users/{userId}/groups", adminuserhandlers.HandleAdminUserGroupsPost(s, s.sessionStore, s, s.database))
		r.Get("/users/{userId}/delete", adminuserhandlers.HandleAdminUserDeleteGet(s, s.database))
		r.Post("/users/{userId}/delete", adminuserhandlers.HandleAdminUserDeletePost(s, s, s.database))
		r.Get("/users/new", adminuserhandlers.HandleAdminUserNewGet(s))
		...
		r.Get("/settings/general", adminsettingshandlers.HandleAdminSettingsGeneralGet(s, s.sessionStore))
		...
		r.Get("/settings/ui-theme", adminsettingshandlers.HandleAdminSettingsUIThemeGet(s, s.sessionStore))
		r.Post("/settings/ui-theme", adminsettingshandlers.HandleAdminSettingsUIThemePost(s, s.sessionStore, s, s.database))
		r.Get("/settings/sessions", adminsettingshandlers.HandleAdminSettingsSessionsGet(s, s.sessionStore))
		r.Post("/settings/sessions", adminsettingshandlers.HandleAdminSettingsSessionsPost(s, s.sessionStore, s, s.database))
		r.Get("/settings/tokens", adminsettingshandlers.HandleAdminSettingsTokensGet(s, s.sessionStore))
		r.Post("/settings/tokens", adminsettingshandlers.HandleAdminSettingsTokensPost(s, s.sessionStore, s, s.database))
		r.Get("/settings/keys", adminsettingshandlers.HandleAdminSettingsKeysGet(s, s.database))
		r.Post("/settings/keys/rotate", adminsettingshandlers.HandleAdminSettingsKeysRotatePost(s, s, s.database))
		r.Post("/settings/keys/revoke", adminsettingshandlers.HandleAdminSettingsKeysRevokePost(s, s, s.database))
		r.Get("/settings/email", adminsettingshandlers.HandleAdminSettingsEmailGet(s, s.sessionStore))
		...
		r.Get("/settings/email/send-test-email", adminsettingshandlers.HandleAdminSettingsEmailSendTestGet(s, s.sessionStore))
		r.Post("/settings/email/send-test-email", adminsettingshandlers.HandleAdminSettingsEmailSendTestPost(s, s.sessionStore, emailValidator, emailSender))
		r.Get("/settings/sms", adminsettingshandlers.HandleAdminSettingsSMSGet(s, s.sessionStore))
...
func (s *Server) jwtAuthorizationHeaderToContext(handler http.Handler) http.Handler {
	return MiddlewareJwtAuthorizationHeaderToContext(handler, s.tokenParser)
}

func (s *Server) requiresAdminScope(handler http.Handler) http.Handler {
	return MiddlewareRequiresScope(handler, s,
