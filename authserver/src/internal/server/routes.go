...
func (s *Server) initRoutes() {
	...
	s.router.Route("/account", func(r chi.Router) {
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/", func(w http.ResponseWriter, r *http.Request) {
			http.Redirect(w, r, lib.GetBaseUrl()+"/account/profile", http.StatusFound)
		})
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/profile", accounthandlers.HandleAccountProfileGet(httpHelper, s.sessionStore, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/profile", accounthandlers.HandleAccountProfilePost(httpHelper, s.sessionStore, authHelper, s.database, profileValidator, inputSanitizer))
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/email", accounthandlers.HandleAccountEmailGet(httpHelper, s.sessionStore, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/email", accounthandlers.HandleAccountEmailPost(httpHelper, s.sessionStore, authHelper, s.database, emailValidator, inputSanitizer))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/email-send-verification", accounthandlers.HandleAccountEmailSendVerificationPost(httpHelper, s.database, emailSender))
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/email-verify", accounthandlers.HandleAccountEmailVerifyGet(httpHelper, authHelper, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/address", accounthandlers.HandleAccountAddressGet(httpHelper, s.sessionStore, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/address", accounthandlers.HandleAccountAddressPost(httpHelper, s.sessionStore, authHelper, s.database, addressValidator, inputSanitizer))
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/phone", accounthandlers.HandleAccountPhoneGet(httpHelper, s.sessionStore, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/phone", accounthandlers.HandleAccountPhonePost(httpHelper, s.sessionStore, authHelper, s.database, phoneValidator))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/phone-send-verification", accounthandlers.HandleAccountPhoneSendVerificationPost(httpHelper, authHelper, s.database, smsSender))
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/phone-verify", accounthandlers.HandleAccountPhoneVerifyGet(httpHelper, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/phone-verify", accounthandlers.HandleAccountPhoneVerifyPost(httpHelper, authHelper, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/change-password", accounthandlers.HandleAccountChangePasswordGet(httpHelper))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/change-password", accounthandlers.HandleAccountChangePasswordPost(httpHelper, authHelper, s.database, passwordValidator))
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/otp", accounthandlers.HandleAccountOtpGet(httpHelper, s.sessionStore, s.database, otpSecretGenerator))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/otp", accounthandlers.HandleAccountOtpPost(httpHelper, s.sessionStore, authHelper, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/manage-consents", accounthandlers.HandleAccountManageConsentsGet(httpHelper, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/manage-consents", accounthandlers.HandleAccountManageConsentsRevokePost(httpHelper, authHelper, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Get("/sessions", accounthandlers.HandleAccountSessionsGet(httpHelper, s.database))
		r.With(jwtSessionToContext).With(requiresAccountScope).Post("/sessions", accounthandlers.HandleAccountSessionsEndSesssionPost(httpHelper, authHelper, s.database))
	...
	s.router.With(jwtSessionToContext).With(requiresAdminScope).Route("/admin", func(r chi.Router) {
