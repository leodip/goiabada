...
func (s *Server) initRoutes() {
	...
	authorizeValidator := core_validators.NewAuthorizeValidator(s.databasev2)
	tokenParser := core_token.NewTokenParser(s.databasev2)
	permissionChecker := core.NewPermissionChecker(s.databasev2)
	tokenValidator := core_validators.NewTokenValidator(s.databasev2, tokenParser, permissionChecker)
	profileValidator := core_validators.NewProfileValidator(s.databasev2)
	emailValidator := core_validators.NewEmailValidator(s.databasev2)
	addressValidator := core_validators.NewAddressValidator(s.databasev2)
	phoneValidator := core_validators.NewPhoneValidator(s.databasev2)
	passwordValidator := core_validators.NewPasswordValidator()
	identifierValidator := core_validators.NewIdentifierValidator(s.databasev2)
	inputSanitizer := core.NewInputSanitizer()

	codeIssuer := core_authorize.NewCodeIssuer(s.databasev2)
	loginManager := core_authorize.NewLoginManager(codeIssuer)
	otpSecretGenerator := core.NewOTPSecretGenerator()
	tokenIssuer := core_token.NewTokenIssuer(s.databasev2, tokenParser)
	emailSender := core_senders.NewEmailSender(s.databasev2)
	smsSender := core_senders.NewSMSSender(s.databasev2)
	userCreator := core.NewUserCreator(s.databasev2)
